...
// checking the smallest file width and comparing it with the target width
...
//             img.onload = function () {
//                 minImageWidth = Math.min(minImageWidth, img.width);
//                 processedCount++;

//                 if (processedCount === imageFiles.length) {
//                     if (imageWidth > minImageWidth) {
//                         setStatus(`Error: Target width should not be larger than the smallest image width (${minImageWidth}px).`);
...
// converting everything to webp and then zipping it
...
//converting everything to webp but only zipping it if there are more than 1 image
    ...
    function convertNextImage() {
        if (index >= imageFiles.length) {
            if (imageFiles.length > 1) {
                ...
                const content = zip.file(`${clientName}_${serviceName}_0001.webp`).async("blob");
                saveAs(content, `${clientName}_${serviceName}.webp`);
